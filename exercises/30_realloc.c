...
static int push(int value)
{
    if (count == cap) {
        ...
        /* TODO: grow `items` to hold new_cap ints.
                 Put the result in a temporary, check it for NULL, and only
                 then assign it to items and update cap.
                 Return -1 if the allocation fails. */
        (void)new_cap;
        return -1;   /* until you implement the growth, this reports
                        "out of memory" - that is expected */
    }
